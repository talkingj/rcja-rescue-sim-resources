...
This page combines the last three pages into the first genuinely useful robot on this site: one
that follows a wall instead of just avoiding the nearest thing in front of it. It takes about 20
...
That's enough to not crash, but it isn't enough to explore a maze, a robot that only reacts to
what's directly in front of it has no sense of direction. **Wall following** fixes that by picking
one wall (say, the one on the right) and steering to keep it at a roughly constant distance:
...
The `stuck_steps` / `escape_steps_left` machinery is there because plain wall-following, on its
own, gets physically wedged in world1's tighter corners. Step 3 shows you exactly how well (and how
badly) this actually worked.
...
rig can do this, your controller can't, that's a supervisor-only capability covered in a later page
on reading the clock and the score) to answer the acceptance question properly: how much of world1
did this robot actually cover in 3 minutes?
...
sample looks like one of two snapshots. Position data confirms it: over the full 180 seconds, the
robot's position stayed within a box roughly 0.21 m by 0.48 m, and it never stopped moving (no
`escaping=yes` lasting more than a couple of steps), but it was tracing the same short loop
repeatedly rather than pushing further into the maze.
...
That's the real, if unglamorous, limit of this approach: it survives, it doesn't get properly lost,
but it isn't yet a maze-solving algorithm.
...
- The real fix for a repeating loop like this is usually to add memory, some way to detect "I'm
  covering the same ground" and deliberately do something different. That's a bigger project than
  this page, but it's exactly the kind of thing a stronger competition robot needs.
