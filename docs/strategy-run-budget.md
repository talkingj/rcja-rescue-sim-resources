...
Track C proved every scoring mechanism works in isolation. This page is about the resource every
team actually runs out of first: time, not code. Every claim on this page traces back to either the
official rules or a real number this site measured while building Track C, no invented pacing
advice.
...
playing time, don't plan around it.

## What running out the clock actually costs you

This is the one most teams get wrong, and it's a real behavior confirmed by reading the supervisor's
...
unsubmitted map costs you both. Running out the clock *with* a submitted map costs you only the
10%, [worth weighing against `rules-scoring.md`'s numbers](rules-scoring.md#bonuses-applied-last)
for your own team's typical score.

## Where the time actually goes, per Track C's own measurements

- **Getting an identification to register needs real margin, not the bare minimum.** [Sending the
  ...
  threshold. Budget a few real seconds of stillness before every report, not the bare `1.0` the
  rules describe, treat that number as a floor, not a target.
- **Exploration is where a real run's time actually disappears, and it can vanish entirely.**
  ...
  page](code-wall-follower.md) already showed this isn't a hypothetical, it's what a reasonable,
...
3. **Stop-and-report time**, several seconds each, every time you find something real, not the bare
   minimum the rules state.
