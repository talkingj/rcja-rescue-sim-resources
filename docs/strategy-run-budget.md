...
first: time, not code. Every claim on this page traces back to either the official rules or a real
...
- **To get an identification to register, budget real margin, not the bare minimum.** [Sending the
  ...
  rules describe. Thus, treat that number as a floor, not a target.
