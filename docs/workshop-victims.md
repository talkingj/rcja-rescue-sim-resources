...
controller](code-victim-detection.md#step-2-write-the-controller) already adapted with the
...
??? question "Why, and what to change"
    ...
    page](code-victim-detection.md#step-3-why-the-official-threshold-didnt-work-here), a
...
2. **In Activity 3, use a fixed, made-up position instead of a real detected one.** Therefore, the
   point is seeing the score change, not solving position estimation live — that's [a much harder,
   still-open problem this site found the hard way](code-complete-run.md).
...
Don't cut the real-threshold discovery in Activity 2, as it's the single most concrete "official
