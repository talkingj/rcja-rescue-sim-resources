...
1. Go to your Erebus folder → **`player_controllers`**.
2. Open **`ExamplePlayerController_updated.py`**. On Windows, right-click → *Open with* → *IDLE* or
   *Notepad*. On a Mac, right-click → *Open With* → *TextEdit*. On Linux, open it in *gedit* or
   *nano*.
...
- The number is in **radians per second** — a way of measuring turning speed.
- **`6.28`** is about **2 × π**, which is one full wheel-turn every second. That's the robot's top
  speed in the sample code.

Make the robot **slower** by halving it.
...
2. **Save the file** (<kbd>Ctrl</kbd>/<kbd>Cmd</kbd> + <kbd>S</kbd>).
...
!!! success "You should now see — and we verified this for real"
    The robot drives the same way but **noticeably slower** — it covers roughly **half the ground**
    in the same time. *(We measured it on a real run: at `6.28` the robot travelled about twice as
    far in ten seconds as it did at `3.14`. Halving the number really does roughly halve the
    speed.)*
...
Try these one at a time — change the number, save, reset, LOAD, start:

- Set `max_velocity = 1.0` — the robot crawls.
- Set `max_velocity = 6.28` again — back to full speed.
- Try a number **bigger** than 6.28, like `10` — but note the sample maze is small, so a very fast
  robot mostly bumps around walls.

You just changed how the robot behaves by editing its code. Everything a competition robot does —
turning, sensing walls, finding victims — is more lines like this one.
...
- **The robot won't move at all after editing** → you may have changed more than the number (a typo
  in the code). Re-open the file and check line 4 reads `max_velocity = 3.14` with nothing else on
  the line changed. If stuck, re-download a fresh copy of the controller.
...
the cameras, and mapping the maze, building on exactly what you just did.
